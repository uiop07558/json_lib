#include "../lib/data_structures/hashtable/map_c.h"
...
static uint32_t findBucket(ds_um_Map_value* map, void* key, uint32_t keySize);

static ds_status rehash(ds_um_Map_value* map, uint32_t cap);

static inline ds_status resize(ds_um_Map_value* map);

static inline uint32_t getPos(ds_um_Map_value* map, void* key, uint32_t keySize);

static inline ds_status insertInternal(ds_um_Map_value* map, void* key, uint32_t keySize, json_Value value, bool strMode);
...
static ds_status rehash(ds_um_Map_value* map, uint32_t cap) {
  ds_um_Map_value newMap;
  newMap.len = 0;
  newMap._delcnt = 0;
  newMap._fillcnt = 0;
  newMap._cap = cap;
  newMap._buf = (ds_um_Node_value*) calloc(newMap._cap, sizeof(ds_um_Node_value));
  if (map->_buf == NULL) {
    return DS_ALLOC_ERROR;
  }

  for (uint32_t i = 0; i < map->_cap; i++) {
    ds_um_Node_value* curNodePtr = map->_buf + i;
    if (curNodePtr->_isFilled && !curNodePtr->_isDeleted) {
      ds_um_insert_value(&newMap, curNodePtr->key, curNodePtr->keySize, curNodePtr->value);
    }
  }
...
static inline uint32_t getPos(ds_um_Map_value* map, void* key, uint32_t keySize) {
  ...
  for (uint32_t i = 0; i < map->_cap; i++) {
    ...
    ds_um_Node_value* curNodePtr = map->_buf + curIndex;
...
ds_status ds_um_initMap_value(ds_um_Map_value* map) {
  map->len = 0;
  map->_delcnt = 0;
  map->_fillcnt = 0;

  map->_cap = __INIT_CAP;
  map->_buf = (ds_um_Node_value*) calloc(map->_cap, sizeof(ds_um_Node_value));
  if (map->_buf == NULL) {
    return DS_ALLOC_ERROR;
  }
  return DS_OK;
...
ds_status ds_um_insert_value(ds_um_Map_value* map, void* key, uint32_t keySize, json_Value value) {
  return insertInternal(map, key, keySize, value, 0);
}

ds_um_ValueStatus_value ds_um_get_value(ds_um_Map_value* map, void* key, uint32_t keySize) {
  uint32_t index = getPos(map, key, keySize);
  ds_um_ValueStatus_value ret;
...
ds_status ds_um_delete_value(ds_um_Map_value* map, void* key, uint32_t keySize) {
  uint32_t index = getPos(map, key, keySize);
...
ds_status ds_um_destroyMap_value(ds_um_Map_value* map) {
  for (uint32_t i = 0; i < map->_cap; i++) {
    ds_um_Node_value* curNode = map->_buf + i;
...
ds_status ds_um_initIterator_value(ds_um_Map_value* map, ds_um_Iterator_value* iter) {
  iter->curIndex = 0;
  iter->_map = map;
  iter->curNode = NULL;
...
ds_status ds_um_insertStrk_value (ds_um_Map_value* map, char* key, json_Value value) {
...
ds_status ds_um_deleteStrk_value (ds_um_Map_value* map, char* key, json_Value value) {
  ...
  ds_um_Node_value* curNode = map->_buf + index;
...
ds_um_ValueStatus_value ds_um_getStrk_value (ds_um_Map_value* map, void* key) {
  return ds_um_get_value(map, key, strlen(key)+1);
}
